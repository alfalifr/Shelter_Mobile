...
class NewsCompositeSource(
    ...
    override suspend fun getNewsList(startTimestamp: String): Result<List<News>> = getDataList(mapOf(
        Const.KEY_TYPE to Const.TYPE_NEWS,
        Const.KEY_TIMESTAMP to startTimestamp,
    ))

    override suspend fun getArticleList(startTimestamp: String): Result<List<News>> = getDataList(mapOf(
        Const.KEY_TYPE to Const.TYPE_ARTICLE,
        Const.KEY_TIMESTAMP to startTimestamp,
    ))

    override suspend fun getArticle(timestamp: String): Result<News> = getDataList(mapOf(
        Const.KEY_TYPE to Const.TYPE_ARTICLE,
        Const.KEY_TIMESTAMP to timestamp,
        Const.KEY_IS_SINGLE to true,
    )).toSingleResult()

    override suspend fun getNews(timestamp: String): Result<News> = getDataList(mapOf(
        Const.KEY_TYPE to Const.TYPE_NEWS,
        Const.KEY_TIMESTAMP to timestamp,
        Const.KEY_IS_SINGLE to true,
    )).toSingleResult()

    override suspend fun searchNews(keyword: String, startTimestamp: String): Result<List<News>> = localSrc.searchNews(keyword, startTimestamp)
